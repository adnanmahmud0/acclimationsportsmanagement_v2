  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-700">
        ...
        <PreviewWrapper onEdit={() => setEditingSection("main")}>
          <div className="relative pt-12 pb-24 bg-[#05070a]">
            <div className="absolute inset-0 z-0">
              <Image src="/effect.png" alt="Bg" fill className="object-cover opacity-30" />
            </div>
              ...
              <div className="space-y-8 max-w-5xl mx-auto text-center">
                <GradientHeader tag="h1" size="lg" className="mb-4 text-center leading-tight">
                  {data.content.mainTitle?.split('\n').map((line, i) => (
                    <React.Fragment key={i}>{line}{i < (data.content.mainTitle?.split('\n').length || 0) - 1 && <br />}</React.Fragment>
                  ))}
                </GradientHeader>
                <p className="text-sm font-bold tracking-[0.3em] uppercase text-white/50 mb-4 max-w-3xl mx-auto">
                  {data.content.subDescription}
                </p>
                <div className="flex flex-col items-center">
                  <ul className="text-left space-y-2">
                    {data.content.points?.map((point: string, i: number) => (
      ...
      <Dialog open={editingSection !== null} onOpenChange={(open) => !open && setEditingSection(null)}>
        <DialogContent className="bg-[#0a0d12] border-white/10 text-white sm:max-w-[600px] rounded-[2rem] overflow-hidden p-0 shadow-3xl">
          <DialogHeader className="p-8 border-b border-white/5 bg-white/5">
            <DialogTitle className="text-xl font-black uppercase italic tracking-tighter">
              Edit <span className="text-blue-500">Page Content</span>
            </DialogTitle>
          </DialogHeader>

          <div className="p-8 space-y-8 max-h-[70vh] overflow-y-auto no-scrollbar">
            <EditField label="Main Heading" type="textarea" height="h-24" value={data.content.mainTitle || ""} onChange={(v) => updateContent("mainTitle", v)} />
            <EditField label="Sub-Description" type="textarea" height="h-32" value={data.content.subDescription || ""} onChange={(v) => updateContent("subDescription", v)} />
            ...
            <div className="space-y-4 pt-4 border-t border-white/5">
              ...
              <div className="space-y-3">
                ...
                <Button variant="outline" size="sm" onClick={() => updateContent("points", [...(data.content.points || []), "New Point"])} className="w-full border-dashed border-white/10 h-11 rounded-xl text-[10px] font-black uppercase tracking-widest">+ Add Point</Button>
              </div>
            </div>

            <div className="space-y-4 pt-4 border-t border-white/5">
              <label className="text-[10px] font-black text-white/40 uppercase tracking-widest block">Negotiation Steps</label>
              <div className="grid gap-4">
                {data.content.processSteps?.map((step: ProcessStep, idx: number) => (
                  <div key={idx} className="bg-white/5 p-4 rounded-2xl space-y-3 border border-white/5">
                    ...
                    }} placeholder="Step Title" className="bg-white/5 border-white/10 text-white rounded-xl h-10 text-xs" />
                    <Input value={step.subtitle} onChange={(e) => {
                      const newSteps = [...(data.content.processSteps || [])]
                      newSteps[idx].subtitle = e.target.value
                      updateContent("processSteps", newSteps)
                    }} placeholder="Step Subtitle" className="bg-white/5 border-white/10 text-white rounded-xl h-10 text-xs" />
                  </div>
                ))}
              </div>
            </div>

            <EditField label="CTA Button Text" value={data.content.ctaText || ""} onChange={(v) => updateContent("ctaText", v)} />
          </div>
