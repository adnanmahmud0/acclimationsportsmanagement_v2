...
export async function generateMetadata() {
  ...
  return buildMetadataFromPage(page, "/personal-branding");
}
      ...
      <div className="container mx-auto px-6 pt-32 pb-24 relative z-10 flex flex-col items-center">
        ...
        <div className="relative w-full max-w-5xl mb-12">
          ...
          <div className="grid grid-cols-1 md:hidden gap-4 w-full">
            {content.metrics?.map((metric: { title: string; value: string }, i: number) => (
              <DataPoint key={i} title={metric.title} value={metric.value} />
            ))}
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-12 items-end">
            ...
            <div className="flex-1 space-y-8">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {content.services?.map((service: { title: string; desc: string }, i: number) => (
                  <ServiceBox key={i} title={service.title} desc={service.desc} />
                ))}
              </div>
            </div>
            ...
            <div className="lg:w-1/3 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                {content.highlights?.map((highlight: { value: string; label: string }, i: number) => (
                  <div key={i} className="bg-[#0a0d12]/95 border border-primary/20 p-8 rounded-2xl shadow-2xl text-center hover:scale-105 transition-all duration-500">
                    <div className="text-4xl font-black text-white mb-2">
                      {highlight.value}
                    </div>
                    <div className="text-[10px] font-black text-white/50 uppercase tracking-[0.4em] leading-relaxed whitespace-pre-line">
                      {highlight.label}
                    </div>
                  </div>
                ))}
        ...
        <div className="pt-12 w-full pb-12 flex flex-col items-center text-center">
